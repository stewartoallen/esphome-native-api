...
class PlaintextFrameHelper extends FrameHelper {
    ...
    deserialize(buffer) {
        ...
        const message = this.buildMessage(messageId, buffer.slice(offset, messageLength + offset));
        if (message) {
            message.length = messageLength + offset;
        } else {
            console.log('MESSAGE BUILD FAIL', { messageId });
        }
        return message;
    }
    ...
    onData(data) {
        ...
        try {
            while ((message = this.deserialize(this.buffer))) {
                ...
                // console.log('-- RECV |', message.constructor.type, message.toObject());
    ...
    sendMessage(message) {
        // console.log('-- SEND |', message.constructor.type, message.toObject());
